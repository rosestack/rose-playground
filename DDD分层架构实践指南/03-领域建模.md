...
public abstract class DomainEvent<T, ID> {
    ...
    protected DomainEvent(T eventData, ID aggregateId) {
...
public class User extends AggregateRoot<String> {
    ...
    public static User create(String username, String email, String password) {
        ...
        // 发布用户创建事件
        user.addDomainEvent(new UserCreatedEvent(user, username, email));
    ...
    public void changeEmail(String newEmail) {
        ...
        // 发布邮箱变更事件
        addDomainEvent(new UserEmailChangedEvent(this, oldEmail, newEmail));
    }
...
public class UserCreatedEventData {
    ...
    public UserCreatedEventData(String username, String email) {
        this.username = username;
        this.email = email;
    }
...
public class UserCreatedEvent extends DomainEvent<UserCreatedEventData, String> {
    public UserCreatedEvent(User user, String username, String email) {
        super(new UserCreatedEventData(username, email), user.getId());
    }
    
    @Override
    public String toString() {
        return String.format("UserCreatedEvent{userId=%s, username=%s, email=%s, occurredOn=%s}", 
                           getAggregateId(), getEventData().getUsername(), getEventData().getEmail(), getOccurredOn());
    }
}
```
 ...
 * @since 1.0.0
 */
public class UserActivatedEvent extends DomainEvent<Void, String> {
    
    public UserActivatedEvent(User user) {
        super(null, user.getId());
    }
 ...
 * @since 1.0.0
 */
public class UserDeactivatedEvent extends DomainEvent<Void, String> {
 ...
 * @since 1.0.0
 */
public class UserEmailChangedEventData {
    /** 原邮箱 */
    ...
    public UserEmailChangedEventData(String oldEmail, String newEmail) {
...
public class UserEmailChangedEvent extends DomainEvent<UserEmailChangedEventData, String> {
    public UserEmailChangedEvent(User user, String oldEmail, String newEmail) {
        super(new UserEmailChangedEventData(oldEmail, newEmail), user.getId());
    }
    
    @Override
    public String toString() {
        return String.format("UserEmailChangedEvent{userId=%s, oldEmail=%s, newEmail=%s, occurredOn=%s}", 
                           getAggregateId(), getEventData().getOldEmail(), getEventData().getNewEmail(), getOccurredOn());
    }
}
```
 ...
 * @since 1.0.0
 */
public class UserPasswordChangedEvent extends DomainEvent<Void, String> {
...
public class UserCreatedEventHandler implements DomainEventHandler<UserCreatedEvent> {
    ...
    public void handle(UserCreatedEvent event) {
        try {
            ...
            // 发送欢迎邮件
            sendWelcomeEmail(event.getAggregateId(), event.getEventData().getEmail());
    ...
    private void logUserCreation(UserCreatedEvent event) {
        ...
        logger.info("用户创建审计日志: userId={}, username={}, email={}, time={}", 
                   event.getAggregateId(), event.getEventData().getUsername(), event.getEventData().getEmail(), event.getOccurredOn());
    }
}
```
...
public class UserEmailChangedEventHandler implements DomainEventHandler<UserEmailChangedEvent> {
    ...
    public void handle(UserEmailChangedEvent event) {
        try {
            ...
            // 发送邮箱变更通知到新邮箱
            sendEmailChangeNotification(event.getAggregateId(), event.getEventData().getNewEmail());
            
            // 发送邮箱变更通知到旧邮箱
            sendEmailChangeNotification(event.getAggregateId(), event.getEventData().getOldEmail());
            
            // 更新相关系统的邮箱信息
            updateRelatedSystems(event.getAggregateId(), event.getEventData().getNewEmail());
    ...
    private void logEmailChange(UserEmailChangedEvent event) {
        ...
        logger.info("用户邮箱变更审计日志: userId={}, oldEmail={}, newEmail={}, time={}", 
                   event.getAggregateId(), event.getEventData().getOldEmail(), event.getEventData().getNewEmail(), event.getOccurredOn());
    }
}
```
