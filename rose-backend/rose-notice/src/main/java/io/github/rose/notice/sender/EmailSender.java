...
public class EmailSender extends AbstractConfigure implements Sender {
    ...
    public void doConfigure(SenderConfiguration config) {
        String host = config.getConfig().get("mail.smtp.host").asText();
        String username = config.getConfig().get("mail.smtp.username").asText();
        String password = config.getConfig().get("mail.smtp.password").asText();
        int port = config.getConfig().get("mail.smtp.port") != null
                ? config.getConfig().get("mail.smtp.port").asInt()
                : 25;
        ...
        props.put(
                "mail.smtp.auth",
                String.valueOf(config.getConfig().get("mail.smtp.auth").asBoolean(true)));
        props.put(
                "mail.smtp.starttls.enable",
                String.valueOf(
                        config.getConfig().get("mail.smtp.starttls.enable").asBoolean(true)));
    ...
    public String send(SendRequest request) {
        String from = config.getConfig().get("mail.smtp.from").asText();
    ...
    private static void doSend(SendRequest request, Session session, JsonNode config) throws MessagingException {
        MimeMessage message = new MimeMessage(session);
        String from = config.get("mail.smtp.from").asText();
        if (from != null) {
        ...
        String subject = config.get("mail.smtp.subject").asText();
