...
public class PaymentController {
    ...
    public ApiResponse<String> createPaymentLink(
            @PathVariable String invoiceId, @RequestBody CreatePaymentLinkRequest request) {
    ...
    /**
     * 支付回调入口（统一回调） paymentMethod 例如：ALIPAY、WECHAT、STRIPE
     */
    ...
    private String extractInvoiceId(String method, Map<String, Object> data) {
        // 通用字段优先
        Object v = data.getOrDefault(
                "invoiceId",
                data.getOrDefault(
                        "invoice_id",
                        data.getOrDefault(
                                "out_trade_no",
                                data.getOrDefault("client_reference_id", data.getOrDefault("invoice", "")))));
    ...
    private String extractTransactionId(String method, Map<String, Object> data) {
        Object v = data.getOrDefault(
                ...
                data.getOrDefault(
                        "transaction_id",
                        data.getOrDefault(
                                "trade_no", data.getOrDefault("payment_intent_id", data.getOrDefault("id", "")))));
        String s = v == null ? "" : v.toString();
        if (s.isEmpty()) {
    ...
    static class CreatePaymentLinkRequest {
        @NotBlank
        @PaymentMethodSubset(anyOf = {PaymentMethod.ALIPAY, PaymentMethod.WECHAT, PaymentMethod.STRIPE})
