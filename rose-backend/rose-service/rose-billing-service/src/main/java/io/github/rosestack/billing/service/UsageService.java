...
public class UsageService extends ServiceImpl<UsageRecordRepository, UsageRecord> {
    ...
    public List<UsageRecord> getUsageHistory(String tenantId, String metricType, int limit) {
        List<UsageRecord> records = usageRepository.findByTenantIdAndMetricTypeOrderByRecordTimeDesc(tenantId, metricType);
     ...
     * 聚合使用量数据
     */
    public Map<String, BigDecimal> aggregateUsageByMetric(String tenantId, LocalDateTime startTime, LocalDateTime endTime) {
        ...
        return stats.stream()
                .collect(java.util.stream.Collectors.groupingBy(
                        ...
                        java.util.stream.Collectors.reducing(
                                BigDecimal.ZERO,
                                stat -> (BigDecimal) stat.get("dailyQuantity"),
                                BigDecimal::add
                        )
                ));
    }
