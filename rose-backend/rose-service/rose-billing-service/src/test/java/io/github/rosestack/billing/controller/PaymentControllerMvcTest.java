...
class PaymentControllerMvcTest {
    ...
    void createPaymentLink_returnsLink() throws Exception {
        Mockito.when(paymentGatewayService.createPaymentLink(
                        eq("inv-1"), any(), eq(io.github.rosestack.billing.payment.PaymentMethod.WECHAT), eq("t-1")))
    ...
    void handleCallback_invalidSignature_returnsError() throws Exception {
        Mockito.when(paymentGatewayService.verifyPaymentCallback(
                        eq(io.github.rosestack.billing.payment.PaymentMethod.ALIPAY), anyMap()))
                .thenReturn(false);
    ...
    void handleCallback_success_callsBillingService() throws Exception {
        Mockito.when(paymentGatewayService.verifyPaymentCallback(
                        eq(io.github.rosestack.billing.payment.PaymentMethod.WECHAT), anyMap()))
                .thenReturn(true);
        ...
        Mockito.verify(billingService)
                .processPayment("inv-1", io.github.rosestack.billing.payment.PaymentMethod.WECHAT, "tx-1");
    }
