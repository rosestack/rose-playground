...
public class TokenKickoutService {
    ...
    public TokenKickoutService(TokenService tokenService, RoseSecurityProperties properties, ApplicationEventPublisher publisher) {
    ...
    public boolean kickByToken(String token) {
        ...
        if (ok && publisher != null) {
            tokenService.resolveUsername(token).ifPresent(user -> publisher.publishEvent(new TokenRevokedEvent(user, token, true)));
        }
        return ok;
    }
