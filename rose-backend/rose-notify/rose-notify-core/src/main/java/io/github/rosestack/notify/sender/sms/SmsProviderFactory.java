...
public class SmsProviderFactory {
    ...
    static {
        ServiceLoader.load(SmsProvider.class)
                .forEach(provider -> PROVIDER_CLASSES.put(provider.getProviderType(), provider.getClass()));
    }

    public static SmsProvider getProvider(String provider, SenderConfiguration config) {
        if (provider == null) {
            throw new NotifyException("短信服务商为空");
        }
        Class<? extends SmsProvider> clazz = PROVIDER_CLASSES.get(provider);
        if (clazz == null) {
            throw new NotifyException("不支持的短信服务商: " + provider);
        }
        ...
        return PROVIDER_CACHE.get(cfgKey, k -> {
            try {
                ...
                throw new NotifyException("创建 SmsProvider 实例失败: " + clazz.getName(), e);
            }
        });
    }
    ...
    private static long getMaxCacheSize() {
        String val = System.getProperty("rose.notify.smsProvider.cache.maxSize", "1000");
        try {
    ...
    private static java.time.Duration getExpireAfterAccessSeconds() {
        String val = System.getProperty("rose.notify.smsProvider.cache.expireAfterAccessSeconds", "1800");
        try {
