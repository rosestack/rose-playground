...
public class RotationAwareFieldEncryptor implements FieldEncryptor {
  ...
  /** 执行加密 */
  private String doEncrypt(String plainText, EncryptType encryptType, KeySpec keySpec)
      throws Exception {
    switch (encryptType) {
      case SM4:
  ...
  /** 执行解密 */
  private String doDecrypt(String cipherText, EncryptType encryptType, KeySpec keySpec)
      throws Exception {
    switch (encryptType) {
      case SM4:
        // SM4 需要16字节的密钥，转换为十六进制字符串
  ...
  /** 对称加密 */
  private String encryptWithSymmetric(String plainText, EncryptType encryptType, byte[] keyBytes)
      throws Exception {
  ...
  /** 对称解密 */
  private String decryptWithSymmetric(String cipherText, EncryptType encryptType, byte[] keyBytes)
      throws Exception {
    SecretKeySpec secretKey = new SecretKeySpec(keyBytes, encryptType.name());
    Cipher cipher = Cipher.getInstance(encryptType.name());
