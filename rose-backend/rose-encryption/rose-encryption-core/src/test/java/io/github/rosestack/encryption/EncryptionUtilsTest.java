...
class EncryptionUtilsTest {
    ...
    void testAESEncryptDecrypt() {
        ...
        // 解密
        String decrypted = EncryptionUtils.decrypt(encrypted, EncryptType.AES, TEST_SECRET_KEY);
    ...
    void testMultipleEncryptionTypes(EncryptType encryptType) {
        String encrypted = EncryptionUtils.encrypt(TEST_PLAIN_TEXT, encryptType, TEST_SECRET_KEY);
        assertNotNull(encrypted);
        assertNotEquals(TEST_PLAIN_TEXT, encrypted);

        String decrypted = EncryptionUtils.decrypt(encrypted, encryptType, TEST_SECRET_KEY);
        assertEquals(TEST_PLAIN_TEXT, decrypted);
    }
    ...
    void testNullAndEmptyValues() {
        // 空字符串
        assertEquals("", EncryptionUtils.encrypt("", EncryptType.AES, TEST_SECRET_KEY));
        assertEquals("", EncryptionUtils.decrypt("", EncryptType.AES, TEST_SECRET_KEY));

        // null值
        assertNull(EncryptionUtils.encrypt(null, EncryptType.AES, TEST_SECRET_KEY));
        assertNull(EncryptionUtils.decrypt(null, EncryptType.AES, TEST_SECRET_KEY));
    }
    ...
    void testEmptySecretKeyThrowsException() {
        assertThrows(EncryptionException.class, () ->
            EncryptionUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, ""));

        assertThrows(EncryptionException.class, () ->
            EncryptionUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, null));
    }
    ...
    void testSM4EncryptionWithJavaScriptEngine() {
        try {
            String encrypted = EncryptionUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.SM4, TEST_SECRET_KEY);
            String decrypted = EncryptionUtils.decrypt(encrypted, EncryptType.SM4, TEST_SECRET_KEY);
            assertEquals(TEST_PLAIN_TEXT, decrypted);
    ...
    void testDifferentTextLengths(String text) {
        String encrypted = EncryptionUtils.encrypt(text, EncryptType.AES, TEST_SECRET_KEY);
        assertNotNull(encrypted);
    ...
    void testPerformanceStats() {
		...
		Map<String, Long> emptyStats = EncryptionMonitorManager.getInstance().getPerformanceStats();
    ...
    void testBatchEncryptDecrypt() {
        ...
        String[] decrypted = EncryptionUtils.decryptBatch(encrypted, EncryptType.AES, TEST_SECRET_KEY);
    ...
    void testEncryptionRandomness() {
        ...
        String encrypted1 = EncryptionUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, TEST_SECRET_KEY);
        String encrypted2 = EncryptionUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, TEST_SECRET_KEY);

        assertNotEquals(encrypted1, encrypted2, "相同明文的两次加密结果应该不同（由于随机IV）");

        // 但解密结果应该相同
        String decrypted1 = EncryptionUtils.decrypt(encrypted1, EncryptType.AES, TEST_SECRET_KEY);
        String decrypted2 = EncryptionUtils.decrypt(encrypted2, EncryptType.AES, TEST_SECRET_KEY);
