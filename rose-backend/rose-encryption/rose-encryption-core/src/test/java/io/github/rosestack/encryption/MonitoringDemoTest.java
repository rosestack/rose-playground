...
class MonitoringDemoTest {
    ...
    void demonstrateMonitoringFeatures() {
        ...
        // 不同算法
        String text1 = EncryptionUtils.encrypt("Hello AES", EncryptType.AES, SECRET_KEY);
        String text2 = EncryptionUtils.encrypt("Hello DES", EncryptType.DES, SECRET_KEY);
        String text3 = EncryptionUtils.encrypt("Hello 3DES", EncryptType.DES3, SECRET_KEY);

        // 不同大小的数据
        String smallData = EncryptionUtils.encrypt("Small", EncryptType.AES, SECRET_KEY);
        String mediumData = EncryptionUtils.encrypt("This is a medium length text for testing encryption performance and monitoring capabilities", EncryptType.AES, SECRET_KEY);

        StringBuilder largeDataBuilder = new StringBuilder();
        for (int i = 0; i < 100; i++) {
            largeDataBuilder.append("Large data chunk ").append(i).append(" ");
        }
        String largeData = EncryptionUtils.encrypt(largeDataBuilder.toString(), EncryptType.AES, SECRET_KEY);

        // 解密操作
        EncryptionUtils.decrypt(text1, EncryptType.AES, SECRET_KEY);
        EncryptionUtils.decrypt(text2, EncryptType.DES, SECRET_KEY);
        EncryptionUtils.decrypt(smallData, EncryptType.AES, SECRET_KEY);

        // 故意触发一些错误
        try {
            EncryptionUtils.decrypt("invalid_base64", EncryptType.AES, SECRET_KEY);
        } catch (Exception e) {
            // 预期的错误
        }

        try {
            EncryptionUtils.encrypt("test", EncryptType.AES, "short"); // 短密钥
        } catch (Exception e) {
            // 预期的错误
        }
        ...
        Map<String, Long> performanceStats = EncryptionMonitorManager.getInstance().getPerformanceStats();
    ...
    void performanceBenchmark() {
        ...
        for (int i = 0; i < iterations; i++) {
            String encrypted = EncryptionUtils.encrypt(testData, EncryptType.AES, SECRET_KEY);
            EncryptionUtils.decrypt(encrypted, EncryptType.AES, SECRET_KEY);
        }
