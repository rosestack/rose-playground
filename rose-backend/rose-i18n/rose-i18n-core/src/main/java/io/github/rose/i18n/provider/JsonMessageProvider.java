 ...
 * @since 1.0.0
 */
public class JsonMessageProvider implements I18nMessageProvider {

    private static final Logger logger = LoggerFactory.getLogger(JsonMessageProvider.class);
    ...
    public String getName() {
        return "JsonMessageProvider";
    }

    @Override
    public boolean supports(I18nProviderConfig config) {
        return "json".equals(config.getType()) || 
               (config.getStringProperty("baseName") != null && 
                config.getStringProperty("format", "").equals("json"));
    }
    ...
    public void initialize(I18nProviderConfig config) {
        ...
        if (baseName == null) {
            throw new IllegalArgumentException("baseName is required for JsonMessageProvider");
        }
        ...
        initialized = true;
        logger.info("JsonMessageProvider initialized: baseName={}, location={}, supportedLocales={}", 
    ...
    @Override
    public void refresh() {
        logger.info("Refreshing JsonMessageProvider");
        messageCache.clear();
        loadAllMessages();
    ...
    public void destroy() {
        ...
        initialized = false;
        logger.info("JsonMessageProvider destroyed");
    }
    ...
    private Map<String, String> loadMessagesForLocale(Locale locale) {
        ...
        try (InputStream inputStream = getResourceInputStream(resourcePath)) {
            ...
            if (flattenKeys) {
                flattenJsonNode("", rootNode, messages);
            } else {
                extractSimpleMessages(rootNode, messages);
            }
            ...
            logger.warn("Failed to load JSON messages for locale {} from {}", locale, resourcePath, e);
            return null;
        }
    }
     ...
     * 扁平化JSON节点
     */
    private void flattenJsonNode(String prefix, JsonNode node, Map<String, String> messages) {
        if (node.isObject()) {
            Iterator<Map.Entry<String, JsonNode>> fields = node.fields();
            while (fields.hasNext()) {
                Map.Entry<String, JsonNode> field = fields.next();
                String key = prefix.isEmpty() ? field.getKey() : prefix + keySeparator + field.getKey();
                flattenJsonNode(key, field.getValue(), messages);
            }
        } else if (node.isArray()) {
            for (int i = 0; i < node.size(); i++) {
                String key = prefix + "[" + i + "]";
                flattenJsonNode(key, node.get(i), messages);
            }
        } else {
            messages.put(prefix, node.asText());
        }
    }
     ...
     * 提取简单消息（不扁平化）
     */
    private void extractSimpleMessages(JsonNode node, Map<String, String> messages) {
        if (node.isObject()) {
            Iterator<Map.Entry<String, JsonNode>> fields = node.fields();
            while (fields.hasNext()) {
                Map.Entry<String, JsonNode> field = fields.next();
                if (field.getValue().isTextual()) {
                    messages.put(field.getKey(), field.getValue().asText());
                }
            }
        }
    }
