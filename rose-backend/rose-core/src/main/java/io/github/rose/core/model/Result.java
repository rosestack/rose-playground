 ...
 *
 * // 失败响应带消息
 * Result<User> errorResult = Result.failure("用户未找到");
 *
 * // 函数式处理
 * Result<String> nameResult = userResult
 *     .map(User::getName)
...
public class Result<T> {
     ...
     * @return A successful Result instance with no data
     */
     ...
     * @return A successful Result instance containing the provided data
     */
     ...
     *
     * @param <T> The type parameter for the result
     * @return A failure Result instance with default error message
     */
     ...
     * @return A failure Result instance with the specified code and message
     */
     ...
     * @param <T>     The type parameter for the result
     * @param message The error message
     * @return A failure Result instance with the specified message
     */
    ...
    public <R> Result<R> map(Function<T, R> mapper) {
        if (isSuccess()) {
            try {
                ...
                return failure("Data transformation failed: " + e.getMessage());
            }
        } else {
     ...
     * is a failure, returns a new failure result with the same error message.
     *
     * @param <R>    The type of the data in the returned Result
     * @param mapper The function that transforms data to another Result
     * @return The Result returned by the mapper function or the original error
     */
    ...
    public Result<T> filter(Predicate<T> predicate, String errorMessage) {
        if (isSuccess() && data != null) {
            try {
                ...
                return failure("Data filtering failed: " + e.getMessage());
            }
        }
        return this;
    }
     ...
     * <p>
     * This method provides a safe way to extract data from a Result with a
