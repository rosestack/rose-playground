 ...
 * <pre>{@code
 * // Object to JSON
 * String json = JacksonUtils.toJsonString(user);
 *
 * // JSON to Object
 * User user = JacksonUtils.parseObject(json, User.class);
 *
 * // JSON to List
 * List<User> users = JacksonUtils.parseArray(json, User.class);
 *
 * // Pretty formatted JSON
 * String prettyJson = JacksonUtils.toPrettyJsonString(user);
 *
 * // File operations
 * JacksonUtils.writeToFile(user, Paths.get("user.json"));
 * User user = JacksonUtils.readFromFile(Paths.get("user.json"), User.class);
 * }</pre>
 *
 * <h3>Thread Safety:</h3>
