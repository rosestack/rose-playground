 ...
 * 审计日志主表实体类
 * <p>
 * 存储审计日志的核心信息，包括事件基本信息、用户信息、HTTP信息、执行结果等。
 * 采用混合模式设计：技术分类使用枚举，具体业务操作使用字符串。
 * 支持多租户、加密存储、完整性保护等特性。
 * </p>
 *
 * @author Rose Team
...
public class AuditLog {
     ...
     * 具体业务操作名称
     */
    @Size(max = 200, message = "操作名称长度不能超过200个字符")
    ...
    @Size(max = 20, message = "操作状态长度不能超过20个字符")
    @TableField("status")
     ...
     * 用户代理简要信息
     */
    @Size(max = 100, message = "用户代理信息长度不能超过100个字符")
     ...
     * 追踪ID（链路追踪）
     */
    @Size(max = 100, message = "追踪ID长度不能超过100个字符")
    @TableField("trace_id")
     ...
     * 哈希值（完整性保护）
     */
    @Size(max = 128, message = "哈希值长度不能超过128个字符")
     ...
     * 前一条记录哈希值（链式完整性保护）
     */
    @Size(max = 128, message = "前一条记录哈希值长度不能超过128个字符")
