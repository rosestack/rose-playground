...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    @ConditionalOnProperty(
            prefix = "rose.web.filter.request-id",
            name = "enabled",
            havingValue = "true",
            matchIfMissing = true)
    public FilterRegistrationBean<TraceIdFilter> requestIdFilter() {
        return FilterRegistrationBeanUtils.createFilterBean(
                new TraceIdFilter(roseWebProperties.getFilter().getExcludePaths()),
                Constants.FilterOrder.REQUEST_FILTER_ORDER);
    }

    @Bean
    @ConditionalOnProperty(
            prefix = "rose.web.filter.caching-request",
            name = "enabled",
            havingValue = "true",
            matchIfMissing = true)
    ...
    public FilterRegistrationBean<XssFilter> xxsFilter() {
        return FilterRegistrationBeanUtils.createFilterBean(
                new XssFilter(roseWebProperties.getFilter().getExcludePaths()), XSS_FILTER_ORDER);
    }
}
