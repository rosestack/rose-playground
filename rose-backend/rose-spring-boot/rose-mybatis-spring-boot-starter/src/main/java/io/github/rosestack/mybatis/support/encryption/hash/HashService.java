...
public class HashService {
    ...
    public boolean verifyHash(String plainText, String storedHash, HashType hashType) {
        if (!StringUtils.hasText(plainText) || !StringUtils.hasText(storedHash)) {
            return false;
        }
    ...
    public HashType getDefaultHashType() {
        RoseMybatisProperties.Encryption.Hash hashConfig = properties.getEncryption().getHash();
    ...
    public String generateHashFieldName(String originalFieldName, String customHashField) {
        if (StringUtils.hasText(customHashField)) {
     ...
     * 根据实体类字段注解生成哈希值
     * <p>
     * 自动从字段的 @EncryptField 注解中获取哈希算法类型
     * </p>
     *
     * @param plainText 明文
     ...
     * 常量时间字符串比较（防时序攻击）
     * <p>
     * 使用常量时间比较算法，避免因为字符串长度或内容差异导致的时序攻击
     * </p>
     *
     * @param a 字符串 A
     * @param b 字符串 B
