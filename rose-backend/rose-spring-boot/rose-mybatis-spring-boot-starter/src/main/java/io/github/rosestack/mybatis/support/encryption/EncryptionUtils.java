 ...
 * 通用加密工具类
 * <p>
 * 提供统一的加密解密功能，支持多种加密算法。
 * 被 FieldEncryptor 和审计模块共同使用，避免代码重复。
 * </p>
 *
 * @author Rose Team
 * @since 1.0.0
...
public final class EncryptionUtils {
    ...
    public static String[] encryptBatch(String[] plainTexts, EncryptType encryptType, String secretKey, boolean failOnError) {
        ...
        for (int i = 0; i < plainTexts.length; i++) {
            results[i] = encrypt(plainTexts[i], encryptType, secretKey, failOnError);
        }
        return results;
    }
    ...
    public static String[] decryptBatch(String[] cipherTexts, EncryptType encryptType, String secretKey, boolean failOnError) {
        ...
        for (int i = 0; i < cipherTexts.length; i++) {
            results[i] = decrypt(cipherTexts[i], encryptType, secretKey, failOnError);
        }
        return results;
    }
    ...
    public static boolean verifyHash(String data, String hash) {
        if (!StringUtils.hasText(data) || !StringUtils.hasText(hash)) {
            return false;
        }
