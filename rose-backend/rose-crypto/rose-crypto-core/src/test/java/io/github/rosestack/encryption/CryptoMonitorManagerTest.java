...
class CryptoMonitorManagerTest {
    ...
    void testBasicPerformanceStats() {
        ...
        CryptoUtils.encrypt("Short", EncryptType.AES, TEST_SECRET_KEY);
    ...
    void testErrorStats() {
        try {
            // 故意使用无效密文触发错误
            CryptoUtils.decrypt("invalid_cipher_text", EncryptType.AES, TEST_SECRET_KEY);
        } catch (Exception e) {
            // 预期的异常
        }
    ...
    void testDataSizeStats() {
        ...
        CryptoUtils.encrypt(largeText.toString(), EncryptType.AES, TEST_SECRET_KEY); // large (>1000字符)
    ...
    void testMonitoringReport() {
        ...
        CryptoUtils.decrypt(encrypted, EncryptType.AES, TEST_SECRET_KEY);

        // 触发一个错误
        try {
            CryptoUtils.decrypt("invalid", EncryptType.AES, TEST_SECRET_KEY);
        } catch (Exception e) {
            // 预期的异常
        }
    ...
    void testClearCache() {
        ...
        // 验证有统计数据
        assertFalse(CryptoMonitorManager.getInstance().getPerformanceStats().isEmpty());

        // 清空缓存
        CryptoMonitorManager.getInstance().clearCache();

        // 验证统计数据已清空
        assertTrue(CryptoMonitorManager.getInstance().getPerformanceStats().isEmpty());
        assertTrue(CryptoMonitorManager.getInstance().getAlgorithmUsageStats().isEmpty());
        assertTrue(CryptoMonitorManager.getInstance().getErrorStats().isEmpty());
        assertTrue(CryptoMonitorManager.getInstance().getDataSizeStats().isEmpty());
    }
    ...
    void testBatchOperationMonitoring() {
        ...
        // 批量解密
        CryptoUtils.decryptBatch(encrypted, EncryptType.AES, TEST_SECRET_KEY);
