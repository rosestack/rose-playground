...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleBusinessException(
    ...
    return ResponseEntity.status(500)
            ...
            .header(RETRY_ALLOWED, String.valueOf(retryService.isRetryable(e)))
            .body(response);
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleValidationException(
    ...
    return ResponseEntity.badRequest()
            .header(ERROR_DETAILS, localizedMessage)
            .header(REQUEST_ID, getRequestId())
            .header(RESPONSE_TIME, String.valueOf(System.currentTimeMillis()))
...
```bash
X-Retry-Allowed: true/false          # 是否允许重试
Retry-After: 60                      # 重试间隔（秒）
X-Error-Type: RATE_LIMIT/SYSTEM      # 错误类型
```
...
1. **异常捕获**：全局异常处理器捕获所有未处理的异常
2. **异常分类**：根据异常类型进行分类处理
3. **重试判断**：判断异常是否可重试，设置相应的响应头
4. **消息国际化**：根据用户语言偏好获取对应的错误消息
5. **响应构建**：构建统一的API响应格式
6. **响应头设置**：设置错误码、请求ID、重试信息等响应头
7. **日志记录**：记录异常信息用于问题排查
8. **响应返回**：返回格式化的错误响应
